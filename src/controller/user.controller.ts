...
const userIdSchema = z.string().refine(value => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value), {
...
export const userById = async (req: Request, res: Response) => {
    try {
        ...
        const result = await db.select().from(users).where(eq(users.id, reqUserId));
        console.log(result);
        if (result.length < 1) {
        ...
        res.status(200).json({ data: result, message: "User fetched Successfully" });
    } catch (err) {
        console.error('Error creating user:', err);
        if (err instanceof z.ZodError) {
...
export const updateUser = async (req: Request, res: Response) => {
    try {
        ...
        const newUser: { id?: string, name?: string, email?: string, password?: string } = req.body;
        console.log(req.body, userId);
        const updatedUser = await db.update(users).set(newUser).where(eq(users.id, userId)).returning();
